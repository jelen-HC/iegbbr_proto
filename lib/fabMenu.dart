...
class FabCircularMenu extends StatefulWidget {
  ...
  FabCircularMenu({
    ...
    this.fabOpenColor,
    this.fabCloseColor,
    this.fabOpenIcon = const Icon(Icons.filter_list),
...
class _FabCircularMenuState extends State<FabCircularMenu>
  ...
  void didChangeDependencies() {
    ...
    ringDiameter = widget.ringDiameter ?? MediaQuery.of(context).size.width * 3;
      ...
      children: <Widget>[
        ...
        Positioned(
          ...
          child: Container(
            ...
            height: scaleAnimation.value * ringDiameter,
            child: CustomPaint(
              foregroundPainter: _InnerRingPainter(
                  ringColor: widget.ringColor,
                  ringWidth: scaleAnimation.value * ringWidth
              ),
            ),
  ...
  List<Widget> _applyTranslations(List<Widget> widgets) {
    return widgets.asMap().map((index, widget) {
      final double angle = widgets.length == 1 ? 45.0 : 90.0 / (widgets.length * 2 - 2) * (index * 1.3);
...
class _RingPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    Paint paint = Paint()
      ...
      ..strokeWidth = ringWidth * 0.4;

    Offset center = Offset(size.width / 1.35, size.height / 2.1);
